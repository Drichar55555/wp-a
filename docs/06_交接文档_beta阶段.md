...
- `next.config.ts` 的 `outputFileTracingIncludes` 必须包含 `./app/generated/prisma/**/*`，否则 serverless function 运行包会缺少 Prisma `.so.node` 引擎
- `vercel.json` 的 `functions.includeFiles` 必须包含 `app/generated/prisma/**`，作为 Vercel function 打包层的兜底
- `package.json` 的生产 build 固定为 `next build --webpack`；不要改回默认 Turbopack build
