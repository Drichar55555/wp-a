...
| # | 任务 | 类型 | 涉及文件 |
|---|------|------|---------|
| 1 | **UI 更新 + 移动端适配** | 前端 | 全部页面组件、`globals.css`、Tailwind 配置 |
| 2 | **加载动画 / 页面转场** | 前端 | `layout.tsx`、各 page.tsx、可能新增动画组件 |
| 3 | **密码复杂度升级** | 后端 | `lib/code.ts`（`newPlainPassword` 改字母表 + 长度） |
| 4 | 收藏夹独立路由页（可选） | 前端 | 新增 `app/collection/page.tsx`，从 `/me` 拆出 |
...
**当前状态**：学生密码是 6 位去混淆字符（`23456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz`），排除 `0/O/1/l/I`。生成逻辑在 `lib/code.ts` 第 29-32 行：
...
**目标**：长度 ≥ 10 位，包含大小写字母 + 数字 + 标点符号，随机生成。

**改法**：
1. 在 `lib/code.ts` 新增一个 `PASSWORD_ALPHABET`，包含 `A-Z a-z 0-9` 加上可打印标点符号（如 `!@#$%^&*()-_=+[]{};:,.<>?`）
2. `newPlainPassword()` 改用 `customAlphabet(PASSWORD_ALPHABET, 10)`（或更长）
3. **保留原来的 `ALPHABET` 不动**——它仍用于生成短码（`code`）和 edit token，这些需要 URL-safe 和视觉无歧义
4. 密码明文在导入/重置时通过 API 返回，打印在纸条上给学生——所以标点符号要选**肉眼能分辨、手输不容易错**的。避免 `|` `\` `` ` `` `'` `"` 这些容易混淆的
5. 改完后跑一次：`npx tsc --noEmit` 确认类型通过，再手动测一次导入→用新格式密码登录→成功

> 密码复杂度变更同时会影响 **操作手册 §6（导入学生）** 和 **管理员操作习惯**——学生纸条上的密码会从 `aB3xK9` 变成 `Xk9#mP2qL5` 这种长度，注意纸条排版空间。
...
### 6.6 头像必填才能发布
`published = true` 时必须有 `avatarUrl`。校验在服务端，前端也有提示。

### 6.7 密码不可逆 + 待升级复杂度
存库的是 scrypt hash，明文只在**生成那一刻**出现一次（导入或重置密码时）。导出 CSV 不含密码。丢了只能走「重置密码」——联系组长 → 开发在 admin 重置 → 生成新明文。

> ⚠️ **当前密码是 6 位去混淆字符，太弱**。§0 任务 3 要求升级为 ≥10 位大小写+数字+标点。改 `lib/code.ts` 的 `newPlainPassword()` 即可，不影响已有数据（旧密码仍能登录，重置时会自动用新规则生成）。
...
### 6.11 系统设置：silent drop
`allowStudentVisibilityControl` 默认 false。当它为 false 时，学生端不显示可见性开关，PATCH 时服务端**静默丢弃** `published` 字段（不报 403）。当它为 true 时，学生才能控制主页公开/私密。
