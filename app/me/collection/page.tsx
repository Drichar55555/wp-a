...
function CollectionList({ items }: { items: CollectionItem[] }) {
  return (
...
export default function CollectionPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (
        ...
        parsed.length === 0 ||
        !isValidItem(parsed[0])
      ) {
        setState("empty");
        return;
      }
      const sorted = (parsed as CollectionItem[]).sort(
  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-md flex-col bg-zinc-50 font-sans dark:bg-black">
      <header className="sticky top-0 z-10 border-b border-zinc-100 bg-white/80 backdrop-blur-md dark:border-zinc-800 dark:bg-black/80">
        <div className="flex items-center gap-3 px-5 py-4">
          <Link
            href="/"
            className="-ml-1 flex h-8 w-8 items-center justify-center rounded-lg text-zinc-500 transition-colors hover:bg-zinc-100 hover:text-zinc-700 dark:hover:bg-zinc-800 dark:hover:text-zinc-300"
            aria-label="返回首页"
          >
            <svg
              className="h-5 w-5"
          ...
          <h1 className="text-[17px] font-semibold text-zinc-800 dark:text-zinc-100">
            我的收藏
          </h1>
          {state === "data" && (
      ...
      {state === "data" && <CollectionList items={items} />}
    </div>
  );
}
